...
struct rim_shader final : public shader{
    ...
    const char* name() override { return "Rim"; }
    ...
    bool fragment(const v3& bar, rgba & col, v3 interpolated_normal, v2 interpolated_uv, const v2& screen_pos) override
    {
        ...
        if (intensity > rim_min && intensity < rim_max){
            intensity = (intensity - rim_min) / (rim_max - rim_min) * intensity * 3;
        } 
        else
        if (intensity < rim_min){
             intensity = 0;
        }
        ...
        auto spec = r.z;
        //clamp to range with hard cutoff
        if (spec > spec_min && spec < spec_max){
             spec = (spec - spec_cut) / (spec_max - spec_cut) * spec;
        }
        else
        if (spec < spec_min){
            spec = 0;
